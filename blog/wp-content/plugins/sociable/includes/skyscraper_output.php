...
function get_rss_node(){
	...
	if ( substr($version,0,1) == 5 &&  isset($skyscraper_options["rss_feed"]) &&  $skyscraper_options["rss_feed"]!="https://"){ 
...
function sc_follow_links($banner = 0){
	...
	foreach($skyscraper_options["follow_us"] as $follow_us){	
		...
		if (!empty($follow_us["active"]) && !empty($follow_us["account"]) ){
			...
			$follow_us["account"] = str_replace("https://", "", $follow_us["account"]);

			$follow_us["account"] =  "https://".$follow_us["account"];
...
function share_links(){
	...
	$share_links = array(
		...
		"twitter" => array('favicon' => 't.png',
            				...
            				'url' => 'https://twitter.com/intent/tweet?text='.urlencode($title).' - '.urlencode($url).' '.urlencode($tag_share).' ',
		...
		"inbound" => array('favicon' => 'inbound.png',
			 			     ...
			 			     'url' => 'https://inbound.org/?url='.$permalink.'&title='.$title.'',
...
function get_tweets_username($username_complete){
	...
	if (function_exists('curl_init')) {
		...
		$url = "https://search.twitter.com/search.json?q=@".$username."&rpp=5&include_entities=true&result_type=mixed";
