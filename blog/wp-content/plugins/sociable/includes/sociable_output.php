...
function send_config_sociable(){
    ...
    if (!empty($sociable_options["pixel"])){
        ...
        curl_setopt($curl, CURLOPT_URL, "https://sociablepixel.blogplay.com/index.php");  
		...
		$html .= '<div class="sociable_tagline">';
				...
				if (isset( $sociable_options['help_grow'] )) {
					...
					if (!empty($sociable_options['help_grow'])){
						...
						$html .= "<a class='sociable_tagline' target='_blank' href='https://blogplay.com'  style='".$addSize."color:#333333;text-decoration:none'>".$tagline."</a>";
					}
					else{
						$html .= $tagline;
					}
        ...
        if ($sitename == "Add to favorites" || $sitename=="More" || $sitename=="vuible"){
			...
			if ($sitename == "More" || $sitename=="vuible"){
				if ($sitename == "More"){
					...
					$link .= "<a onClick=\"javascript:var ipinsite='Good%20Vibes.%20Vuible.com',ipinsiteurl='https://vuible.com/';(function(){if(window.ipinit!==undefined){ipinit();}else{document.body.appendChild(document.createElement('script')).src='https://vuible.com/wp-content/themes/ipinpro/js/ipinit.js';}})();\" style=\"cursor:pointer\" rel=\"nofollow\" title=\"Vuible.com | Share positive messages (images and videos only)\">" . $linkitem . "</a></li>";					
				}
			}else{
	...
	foreach ($sociable_known_sites as $sitename => $val){
        ...
        if ($sitename == "Add to favorites" || $sitename=="More" || $sitename=="vuible"){
			...
			if ($sitename == "More" || $sitename=="vuible"){
				...
				$link .= "<a onClick=\"javascript:var%20ipinsite='Good%20Vibes.%20Vuible.com',ipinsiteurl='https://vuible.com/';(function(){if(window.ipinit!==undefined){ipinit();}else{document.body.appendChild(document.createElement('script')).src='https://vuible.com/wp-content/themes/ipinpro/js/ipinit.js';}})();\" style=\"cursor:pointer\" rel=\"nofollow\" title=\"Vuible.com | Share positive messages (images and videos only)\">" . $linkitem . "</a></li>";					
				}
			}else{
