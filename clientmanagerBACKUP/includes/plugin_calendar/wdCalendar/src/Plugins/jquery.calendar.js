...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        var __SCOLLEVENTTEMP = "<DIV style=\"WIDTH:${width};top:${top};left:${left};\" class=\"chip chip${i} ${chipclass} ${drag}\" title=\"${title}\">${data}<DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}\" class=ct>&nbsp;</DIV><DL style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; HEIGHT: ${height}px; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\"><DT style=\"BACKGROUND-COLOR:${bgcolor2}\">${starttime} - ${endtime} ${icon}</DT><DD class=\"calendar_content\"><span class=\"event_details\">${content}</SPAN><span class=\"customer\">${customer}</span><span class=\"staff\">${staff}</span></DD><DIV class='resizer' style='display:${redisplay}'><DIV class=rszr_icon>&nbsp;</DIV></DIV></DL><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BACKGROUND-COLOR:${bgcolor1}; BORDER-TOP-COLOR: ${bdcolor}; BORDER-RIGHT-COLOR: ${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb1>&nbsp;</DIV><DIV style=\"BORDER-BOTTOM-COLOR:${bdcolor}; BORDER-TOP-COLOR:${bdcolor}; BORDER-RIGHT-COLOR:${bdcolor}; BORDER-LEFT-COLOR:${bdcolor}\" class=cb2>&nbsp;</DIV></DIV>";
        var __ALLDAYEVENTTEMP = '<div class="rb-o ${eclass} ${chipclass}" id="${id}" title="${title}" style="color:${color};">${data}<div class="${extendClass} rb-m" style="background-color:${color}">${extendHTML}<div class="rb-i">${content2}</div></div></div>';
        ...
        function BuildDayEvent(theme, e, index) {
            ...
            p.content = e.event[1];
            p.customer = typeof e.event[11] == 'undefined' ? "" : ( e.event[11] == "" ? "" : e.event[11] + "<br/>" );
            p.staff = typeof e.event['staff'] == 'undefined' ? "" : ( e.event['staff'] == "" ? "" : e.event['staff'] + "<br/>" );
            p.title = getTitle(e.event);
            p.data = '<input type="hidden" class="dhdV" value="'+JSON.stringify(e.event).replace(/"/g, "&#34;")+'">';//e.event.join("$");
            var icons = [];
        ...
        function Tp(temp, dataarry) {
            return temp.replace(/\$\{([\w]+)\}/g, function(s1, s2) { var s = dataarry[s2]; if (typeof (s) != "undefined") { return s2 == 'content'||s2 == 'title' ? $('<div/>').text(s).html() : s; } else { return s1; } });
        }
        ...
        function getdata(chip) {
            var hddata = chip.find("input.dhdV");
            if (hddata.length == 1) {
                var str = hddata.val();
                //console.debug(str);
                //return parseED(str.split("$"));
                return parseED(JSON.parse(str));
            }
            return null;
        }
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && (data[8] == 1 || data[8] == 2) && option.readonly != true) {
                    //console.log(data);
                    $('#bbit-cs-buddle').remove();
                    var csbuddle = typeof data.bubble != 'undefined' ? data.bubble : '<div id="bbit-cs-buddle" style="z-index: 1080; width: 400px;visibility:hidden;" class="bubble"><table class="bubble-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="bubble-cell-side"><div id="tl1" class="bubble-corner"><div class="bubble-sprite bubble-tl"></div></div><td class="bubble-cell-main"><div class="bubble-top"></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div>  <tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div><div class="cb-root"><table class="cb-table" cellSpacing="0" cellPadding="0"><tbody>' +
                        '<tr>' +
                        '<td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><div id="bbit-cs-what" title="'
                    	+ i18n.xgcalendar.click_to_detail + '" class="textbox-fill-div lk" style="cursor:pointer;"></div></div></div></td></tr><tr><td class=cb-value><div id="bbit-cs-buddle-timeshow"></div></td>' +
                        '</tr>' +
                        '<tr><td class=cb-value><div id="bbit-cs-customer-link"></div></td>' +
                        '</tr>' +
