...
','File Comment: {FILE_NAME} {CUSTOMER_NAME}',array(
    ...
    public function pre_menu(){
        ...
        if($this->can_i('edit','Files') || $this->can_i('view','Files')){
            ...
            if(isset($_REQUEST['customer_id']) && $_REQUEST['customer_id'] && $_REQUEST['customer_id']!='new'){
                // how many files?
                $files = $this->get_files(array('customer_id'=>$_REQUEST['customer_id'],'bucket_parent_file_id'=>0));
                $name = _l('Files');
                if(count($files)){
    ...
    Customer: <strong>{CUSTOMER_NAME} is awesome!</strong> <br/>
	...
	<p><input id="reviewed" type="checkbox" name="file_approve_go" value="yes">   I have reviewed my print proof. </p>
    <p><input type="checkbox" name="file_approve_contact" value="yes">   I confirm that all contact information (email, phone, address, etc) is correct. </p>
    <p><input type="checkbox" name="file_approve_spelling" value="yes">   I confirm that the spelling is correct in the entire file. </p>
    <p><input type="checkbox" name="file_approve_colors" value="yes">   I confirm that colors and design are according to my specifications. </p>
     <p><input type="checkbox" name="file_approve_final" value="yes">  I have carefully examined Markit Medias proof and determined that the proof is ready for press. I understand that my product will not be printed until this form is completed. Markit Media is not responsible for any errors that have been approved in the above form. Timely completion of this proof form will ensure a quick turn-around time for project. If corrections need to be made to above proof DO NOT APPROVE FOR PRESS. Click the corrections button and give description of the necessary corrections. </p>
    <p><input id="approvalSubmit" type="submit" name="file_approve" value="Approve File" class="submit_button save_button"></p>    
