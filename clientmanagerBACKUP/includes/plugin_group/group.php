...
/** 
  ...
  * Package Date: 2015-04-27 08:13:58 
...
class module_group extends module_base{
                    ...
                    foreach($group_items as $group_item){
                        ...
                        <tr id="group_<?php echo $group_id;?>">
                            <th class="width1">
                                ...
                                <input type="checkbox" name="used_group_module[<?php echo $owner_table;?>][<?php echo $group_id;?>]" id="groupchk<?php echo $owner_table.$group_id;?>" value="<?php echo htmlspecialchars($owner_id);?>" <?php // check it if the user is a member of this group.
                                    ...
                                    ?>>
                                <?php }else{ ?>
                                    <input type="checkbox" name="group" value="<?php echo htmlspecialchars($owner_id);?>" <?php // check it if the user is a member of this group.
                                    $sql = "SELECT * FROM `"._DB_PREFIX."group_member` gm WHERE `group_id` = $group_id AND `owner_id` = '".mysql_real_escape_string($owner_id)."' AND owner_table = '".mysql_real_escape_string($owner_table)."'";
                                    $res = query($sql);
                                    if(mysql_num_rows($res)){
                                        echo ' checked';
	...
	public static function get_groups($owner_table=false){
        ...
        if($owner_table){
            $sql .= " WHERE g.owner_table = '".mysql_real_escape_string($owner_table)."'";
        }
                        ...
                        if((int)$group_id>0 && $flag == 'yes'){
                            ...
                            foreach($rows as $row){
                                ...
                                if($owner_id>0 && strlen($owner_table)>0){
                                    ...
                                    //$sql .= ", `db_fields` = '".mysql_real_escape_string(serialize(self::$pagination_options))."' ";
                                    query($sql);
                                }
                            }
                        }
