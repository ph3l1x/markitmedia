...
class sidebar_generator {
	...
	public static $theme_text_domain = 'MX';
	...
	public static function add_sidebar(){
		...
		if(isset($sidebars[$id])){
			die('alert("'.__('Sidebar already exists, please use a different name.',sidebar_generator::$theme_text_domain).'")');
		}
	...
	public static function remove_sidebar(){
		...
		$id = esc_html(sidebar_generator::name_to_class($name));
		if(!isset($sidebars[$id])){
			die('alert("'.__('Sidebar does not exist.',sidebar_generator::$theme_text_domain).'")');
		}
	...
	public static function admin_menu(){
		...
		add_submenu_page( 'mx_setting_page',
							__('Sidebars',sidebar_generator::$theme_text_domain), __('Sidebars',sidebar_generator::$theme_text_domain), 'manage_options', 'sidebars_page', array('sidebar_generator','admin_page'));
	}
	...
	public static function admin_page(){
		...
		<script>
			...
			function remove_sidebar_link(name,num){
				var str = '<?php _e("Are you sure you want to remove",sidebar_generator::$theme_text_domain); ?>' + ' ' + name + '?\n' + '<?php _e("This will remove any widgets you have assigned to this sidebar.",sidebar_generator::$theme_text_domain);?>';
				answer = confirm(str);
				if(answer){
					//alert('AJAX REMOVE');
		...
		<div class="wrap">
			<h2><?php _e('Sidebar Generator',sidebar_generator::$theme_text_domain); ?></h2>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php _e('Name',sidebar_generator::$theme_text_domain); ?></th>
					<th><?php _e('CSS class',sidebar_generator::$theme_text_domain); ?></th>
					<th><?php _e('Remove',sidebar_generator::$theme_text_domain); ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
            ...
            <p class="sidebar-tip"><?php esc_html( _e('No Sidebars defined!',sidebar_generator::$theme_text_domain)); ?></p>
            <br /><br />
            <div class="add_sidebar">
            <h4><?php _e('Add Sidebar:',sidebar_generator::$theme_text_domain); ?> </h4>
            	<input id="sidebar-name" type="text" class="regular-text" placeholder="<?php esc_attr(_e('Input sidebar name',sidebar_generator::$theme_text_domain)); ?>" />
            </div>
			<p class="submit">
            	<input type="button" onclick="add_sidebar_link();" class="button-primary" value="<?php esc_attr(_e('Add New Sidebar',sidebar_generator::$theme_text_domain)); ?>" />
            </p>
		</div>
		<?php
	}
