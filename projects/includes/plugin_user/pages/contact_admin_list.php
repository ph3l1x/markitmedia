...
$module->page_title = _l($contact_type.' Contacts');
...
if(!isset($search[$use_master_key]) || !$search[$use_master_key]){
    ...
    if(class_exists('module_security',false)){
        ...
        module_security::check_page(array(
            ...
            'page_name' => 'All '.$contact_type.' Contacts',
...
if(class_exists('module_import_export',false) && module_user::can_i('view','Export '.$contact_type.' Contacts')){
    ...
    module_import_export::enable_pagination_hook(
        ...
        array(
            'name' => $contact_type.' Contact Export',
            'fields'=>array(
                $contact_type.' Contact ID' => 'user_id',
                'First Name' => 'name',
                'Last Name' => 'last_name',
                $contact_type.' ID' => $contact_module_name.'_id',
                $contact_type.' Name' => $contact_module_name.'_name',
...
$heading = array(
    'main' => true,
    'type' => 'h2',
    'title' => _l( ($show_customer_details ? 'All ' : '') . $contact_type.' Contacts'),
    'button' => array(

    )
);
if(isset($search[$use_master_key]) && $search[$use_master_key] && module_user::can_i('create','Contacts',$contact_type)){
