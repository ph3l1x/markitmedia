...
/** 
  ...
  * Package Date: 2015-01-24 12:42:06 
        ...
        if(preg_match_all('#<input[^>]*type="text"[^>]*>#imsU',$html_data,$matches)){
	        foreach($matches[0] as $match){
                ...
                if($encrypt_field_id){
                    ...
                    $html_data = preg_replace('#'.preg_quote($match,'#').'#msU',$replace,$html_data);
                    ...
                    $html_data = preg_replace('#'.preg_quote($match,'#').'#msU',$match.' &nbsp; '.$button,$html_data);
                }
            }
        }
