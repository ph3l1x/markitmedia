...
/** 
  ...
  * Package Date: 2015-01-24 12:42:09 
...
class module_extra extends module_base{
                    ...
                    foreach($default_fields as $default){
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                module_form::generate_form_element(array(
                                    'type' => $field_type,
                                    'name' => 'extra_'.$owner_table.'_field[new'.$defaultid.'][val]',
                                    'value' => '',
                                    'class' => 'extra_value_input',
                                    'id' => $id,
                                ));
    ...
    public static function get_defaults($owner_table=false) {
        ...
        $sql = "SELECT `extra_default_id`,`extra_key`, `order`, `display_type`, `owner_table`, `searchable`, `field_type` FROM `"._DB_PREFIX."extra_default` e $where ORDER BY e.`order` ASC";
        foreach(qa($sql) as $r){
    ...
    public static function print_table_header($owner_table,$options=array())
    {
        ...
        if(self::can_i('view','Extra Fields')){
            ...
            foreach($column_headers as $column_header){
                ...
                ?>
                <th class="extra_column" <?php echo isset($this_options['style']) ? ' style="'.$this_options['style'].'"' : '';?>>
